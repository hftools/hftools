...
from hftools.math import dBinv, dB_angle_to_complex, mag_angle_to_complex, re_im_to_complex
from hftools.dataset import DataDict, DataBlock, DimSweep, DimRep, hfarray, make_matrix, DimMatrix_i, DimMatrix_j
from hftools.file_formats.common import Comments, db_iterator, make_col_from_matrix
from hftools.file_formats.readbase import Stream, Optional, ManyOptional, ManyOptional, One, Token
from hftools.file_formats.readbase import combine_elements_to_matrix, ReadFileFormat
        ...
        for idx, rad in enumerate(stream):
            ...
            if not rad: #skip empty lines
                ...
                if is_touchstone(None, rad):
                    ...
                    raise TouchstoneError("# format is invalid on line: %s"%lineno)
            else:
        ...
        for token, lineno, rad in stream:
            if token == "Comments":
                ...
                if info is None:
                    ...
                    raise TouchstoneError("Second # info at lineno: %d"%lineno)
            else:
...
reg_touch = re.compile(r"\s*#\s*[kmgtp]?hz\s+[szygh]\s+([a-z][a-z])(\s+r\s+[0-9]+)?", re.I)
...
def proc_info(info, f, data, fn, noisedata):
    ...
    if len(info_list) == 5:
        ...
        raise TouchstoneError("# format should only have 5 values this one has %d"%len(info_list))
    z0 = float(z0)
    ...
    diminfo = (DimSweep("freq", f, unit="Hz"), )
    if fn is not None and len(f) == len(fn) and np.allclose(f, fn):
        noisediminfo = diminfo
    elif fn is None:
        noisediminfo = None
    else:
        noisediminfo = (DimSweep("freqn", fn, unit="Hz"), )
    out = make_matrix(out, diminfo)
    f = hfarray(f, diminfo)
    db = DataBlock()
    db[info[1]] = out
    if noisedata is not None:
        db.Rn = hfarray(noisedata[..., -1], dims=noisediminfo) * z0
        db.Fmin = 10 ** (hfarray(noisedata[..., 0], dims=noisediminfo) / 10)
        db.Gopt = hfarray(convfunc[datatype](noisedata[:, 1],  noisedata[:, 2]), dims=noisediminfo)
    db.Z0 = hfarray(z0, unit="Ohm")
    return db


def format_touchstone_block(sweepvars, header, fmts, columns, blockname, comments):
    for comment in comments.fullcomments:
        yield ["!"+comment]
    yield [comments.property.get("INFO", ["#HZ S RI R 50"])[0]]
    header, columns = make_col_from_matrix(header, columns, "%s[%s,%s]", fortranorder=True)
...
def read_touchstone(filnamn, make_complex=True, property_to_vars=True,
    ...
    return ReadTouchstoneFileFormat.read_file(filnamn, make_complex=make_complex,
                property_to_vars=property_to_vars, guess_unit=guess_unit,
                normalize=normalize, make_matrix=make_matrix, merge=merge, verbose=verbose)


if __name__ == "__main__":
    a = read_touchstone("tests/testdata/touchstone/test4.s2p", make_complex=True, property_to_vars=True,
                guess_unit=True, normalize=True, make_matrix=True,
    ...
    d.S = hfarray([[[1+1j, 1+2j], [2+1j, 2+2j]]]*3, dims=(fi, DimMatrix_i("i", 2), DimMatrix_j("j", 2),))
